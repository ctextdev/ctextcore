...
EXPECTED_OUTPUT_FILE_NAME = ".lemma.output.expected."
...
LOGGER.info("Testing LEMMA for the followig languages: {0}".format(languages))

def download_lemma_model(lang, server, retry_count=1):
    """
    Downloads LEMMA model if not available, with retries.
    """
    for attempt_count in range(retry_count + 1):
        # Get updated list of available technologies 
        technologies = server.list_available_techs()
        print(technologies)
        if lang in technologies.get("lemma", []):
            print(f"LEMMA model for language {lang} is already available.")
            return
        print(f"{'Attempting' if attempt_count == 0 else 'Retrying'} download of Lemma model for language: {lang}, Attempt {attempt_count+1}")
        try:
            server.download_model(language=lang, tech="lemma")
...
for lang in languages:
    download_lemma_model(lang, server, retry_count=1)
    if lang not in technologies["lemma"]:
        server.download_model(language=lang, tech="lemma")
    for level in levels:
        EXPECTED_OUTPUT_DIR = CURRENT_DIR+"/data/expected/{0}_level/lemma/".format(level)  # Check
        for output_format in formats:
            ...
            # Check if model is installed for language
            if level=="line":
                ...
                server_output = server.process_text(text_input=input_file_all_lines, tech="lemma", language=lang, output_format=output_format)
            elif level=="file":
                server_output = server.process_text(text_input=Path(INPUT_DIR,str(lang+INPUT_FILE_NAME)), tech="lemma", language=lang,output_format=output_format)
...
def test_process_string(lang,test_input, expected):
    """LEMMA Tagger test_process_string function"""
    assert test_input == expected,"LEMMA failed for {0}".format(lang)