...
EXPECTED_OUTPUT_FILE_NAME = ".pos.output.expected."
    ...
    for level in levels:
        ...
        for output_format in formats:
            ...
            if level=="line":
                ...
                server_output = server.process_text(text_input=input_file_all_lines, tech="pos", language=lang, output_format=output_format)
            elif level=="file":
                server_output = server.process_text(text_input=Path(INPUT_DIR,str(lang+INPUT_FILE_NAME)), tech="pos", language=lang,output_format=output_format)
...
def test_process_string(lang,test_input, expected):
    """POS Tagger test_process_string function"""
    assert test_input == expected,"POS failed for {0}".format(lang)