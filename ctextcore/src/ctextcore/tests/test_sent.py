"""Sentence Seperator Pytest Unit Test"""
...
EXPECTED_OUTPUT_FILE_NAME = ".sent.output.expected."
...
LOGGER.info("Testing Sentence Seperator for the followig languages: {0}".format(languages))

def download_sent_model(lang, server, retry_count=1):
    """
    Downloads Sentence Seperator model if not available, with retries.
    """
    for attempt_count in range(retry_count + 1):
        # Get updated list of available technologies 
        technologies = server.list_available_techs()
        print(technologies)
        if lang in technologies.get("sent", []):
            print(f"Sentence Seperator model for language {lang} is already available.")
            return
        print(f"{'Attempting' if attempt_count == 0 else 'Retrying'} download of Sentence Seperator model for language: {lang}, Attempt {attempt_count+1}")
        try:
            server.download_model(language=lang, tech="sent")
...
for lang in languages:
    download_sent_model(lang, server, retry_count=1)
    #if lang not in technologies["sent"]:
    #    LOGGER.warning("Could not test {0} for {1} because the model is not installed yet".format("sent",lang))
    #    continue
    for level in levels:
        EXPECTED_OUTPUT_DIR = CURRENT_DIR+"/data/expected/{0}_level/sent/".format(level)  # Check
        for output_format in formats:
            ...
            if level=="line":
                ...
                server_output = server.process_text(text_input=input_file_all_lines, tech="sent", language=lang, output_format=output_format)
            elif level=="file":
                server_output = server.process_text(text_input=Path(INPUT_DIR,str(lang+INPUT_FILE_NAME)), tech="sent", language=lang,output_format=output_format)
...
def test_process_string(lang,test_input, expected):
    """Sentence Seperator test_process_string function"""
    assert test_input == expected,"SENT failed for {0}".format(lang)