...
EXPECTED_OUTPUT_FILE_NAME = ".morph.output.expected."
...
LOGGER.info("Testing MORPH for the followig languages: {0}".format(languages))

def download_morph_model(lang, server, retry_count=1):
    """
    Downloads MORPH model if not available, with retries.
    """
    for attempt_count in range(retry_count + 1):
        # Get updated list of available technologies 
        technologies = server.list_available_techs()
        print(technologies)
        if lang in technologies.get("morph", []):
            print(f"MORPH model for language {lang} is already available.")
            return
        print(f"{'Attempting' if attempt_count == 0 else 'Retrying'} download of MORPH model for language: {lang}, Attempt {attempt_count+1}")
        try:
            server.download_model(language=lang, tech="morph")
...
for lang in languages:
    download_morph_model(lang, server, retry_count=1)
    if lang not in technologies["morph"]:
        server.download_model(language=lang, tech="morph")
    for level in levels:
        EXPECTED_OUTPUT_DIR = CURRENT_DIR+"/data/expected/{0}_level/morph/".format(level)  # Check
        for output_format in formats:
            ...
            if level=="line":
                ...
                server_output = server.process_text(text_input=input_file_all_lines, tech="morph", language=lang, output_format=output_format)
            elif level=="file":
                server_output = server.process_text(text_input=Path(INPUT_DIR,str(lang+INPUT_FILE_NAME)), tech="morph", language=lang,output_format=output_format)
...
def test_process_string(lang,test_input, expected):
    """MORPH test_process_string function"""
    assert test_input == expected,"MORPH failed for {0}".format(lang)