...
EXPECTED_OUTPUT_FILE_NAME = ".upos.output.expected."
...
LOGGER.info("Testing UPOS Tagger for the followig languages: {0}".format(languages))

def download_upos_model(lang, server, retry_count=1):
    """
    Downloads UPOS model if not available, with retries.
    """
    for attempt_count in range(retry_count + 1):
        # Get updated list of available technologies 
        technologies = server.list_available_techs()
        print(technologies)
        if lang in technologies.get("upos", []):
            print(f"UPOS model for language {lang} is already available.")
            return
        print(f"{'Attempting' if attempt_count == 0 else 'Retrying'} download of UPOS model for language: {lang}, Attempt {attempt_count+1}")
        try:
            server.download_model(language=lang, tech="upos")
...
for lang in languages:
    download_upos_model(lang, server, retry_count=1)
    if lang not in technologies["upos"]:
        server.download_model(language=lang, tech="upos")
    for level in levels:
        EXPECTED_OUTPUT_DIR = CURRENT_DIR+"/data/expected/{0}_level/upos/".format(level)  # Check
        for output_format in formats:
            ...
            if level=="line":
                ...
                server_output = server.process_text(text_input=input_file_all_lines, tech="upos", language=lang, output_format=output_format)
            elif level=="file":
                server_output = server.process_text(text_input=Path(INPUT_DIR,str(lang+INPUT_FILE_NAME)), tech="upos", language=lang,output_format=output_format)
...
def test_process_string(lang,test_input, expected):
    """UPOS Tagger test_process_string function"""
    assert test_input == expected,"UPOS failed for {0}".format(lang)