...
def download_lid_model(lang, server, retry_count=1):
    """
    Downloads LID model if not available, with retries.
    """
    for attempt_count in range(retry_count + 1):
        # Get updated list of available technologies 
        technologies = server.list_available_techs()
        print(technologies)
        if lang in technologies.get("lid", []):
            print(f"LID model for language {lang} is already available.")
            return
        print(f"{'Attempting' if attempt_count == 0 else 'Retrying'} download of LID model for language: {lang}, Attempt {attempt_count+1}")
        try:
...
for lang in languages:
    download_lid_model(lang, server, retry_count=1)
